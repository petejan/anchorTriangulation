...
with open(fn , 'rt' , encoding="iso-8859-1") as f:
    ...
    for line in f:
        ...
        if matchobj:
            #print("Match Line", matchobj.group(4))
            lat = np.float64(matchobj.group(2))
            lon = np.float64(matchobj.group(3))
            txt = matchobj.group(4).strip()
            if (line.find('Anchor') >= 0) | (i == 0): # get position of anchor drop, or first record
                #print('anchor', i, matchobj)
                anchor_drop_lat = lat
                anchor_drop_lon = lon
                anchor_utm = utm.from_latlon(anchor_drop_lat, anchor_drop_lon);
                
            match_range = range_exp.match(txt)
            #print(txt, match_range)
            if match_range:
                #print (match_range.group(1)) 
                t.append(float(match_range.group(1)))
                utmloc.append(utm.from_latlon(lat, lon, force_zone_number = anchor_utm[2]))
            else:
                if '--.---' not in txt:
                    list_pre.append({'ts': matchobj.group(1), 'lat': lat, 'lon': lon, 'txt': txt})
            i = i + 1
...
with PdfPages(fn+'.pdf') as pdf:
    ...
    for l in list_pre:
        ...
        pre_dist = np.sqrt((pre_utm[0] - anchor_utm[0] + x1)**2 + (pre_utm[1] - anchor_utm[1] + y1)**2)
